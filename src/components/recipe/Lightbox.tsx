...
export function Lightbox({
  ...
  const ctrl =
    'flex h-12 w-12 cursor-pointer items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 hover:text-white focus-visible:ring-2 focus-visible:ring-white/60';
  ...
  return createPortal(
    ...
    <div
      ...
      aria-modal="true"
      onClick={onClose}
      className="fixed inset-0 z-[100] flex cursor-zoom-out flex-col items-center justify-center gap-4 bg-black/70 p-6 backdrop-blur-sm"
    >
      <Button variant="ghost" size="icon" aria-label="Schließen"
        onClick={(e) => { stop(e); onClose(); }} className={`absolute right-5 top-5 ${ctrl}`}>
        <X className="h-6 w-6" />
      </Button>

      {/* Image rendered as large as possible inside a fixed window-relative box. */}
      <div className="relative flex w-full items-center justify-center">
        ...
        <img
          ...
          alt={imageName(id)}
          onClick={stop}
          className="max-h-[50vh] max-w-[50vw] cursor-default rounded-lg object-contain shadow-2xl"
        />
        {n > 1 && (
      ...
      <p onClick={stop} className="cursor-default text-center text-white">
        {imageName(id)}
        {n > 1 && <span className="ml-2 text-white/60">{index + 1} / {n}</span>}
      </p>
      ...
      {n > 1 && (
        <div
          ref={stripRef}
          onClick={stop}
          className="flex max-w-[60vw] cursor-default gap-2 overflow-x-auto [scrollbar-width:none]"
        >
          {images.map((imgId, i) => (
            <button
              key={imgId}
              ...
              className={`h-16 w-16 shrink-0 cursor-pointer overflow-hidden rounded-md transition ${
